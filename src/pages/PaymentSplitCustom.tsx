...
interface ItemAssignment {
  [key: string]: number; // personIndex -> quantity
}

export default function PaymentSplitCustom() {
  const { items, totalPrice } = useCart();
  const navigate = useNavigate();
  const [numPeople, setNumPeople] = useState<string>('2');
  const [setupComplete, setSetupComplete] = useState(false);
  const [itemAssignments, setItemAssignments] = useState<{ [itemId: string]: ItemAssignment }>({});
  ...
  const handleItemAssignment = (itemId: string, personIndex: number, quantity: number) => {
    setItemAssignments(prev => ({
      ...prev,
      [itemId]: {
        ...prev[itemId],
        [personIndex]: quantity
      }
    }));
  };
  ...
  const calculatePersonTotal = (personIndex: number) => {
    let subtotal = 0;
    items.forEach(item => {
      const qty = itemAssignments[item.id]?.[personIndex] || 0;
  ...
  const handleProceedToPayment = () => {
    ...
    const allAssigned = items.every(item => {
      const assigned = Object.values(itemAssignments[item.id] || {}).reduce((sum, qty) => sum + qty, 0);
      return assigned === item.quantity;
    });

    if (!allAssigned) {
      toast({ title: 'Incomplete assignment', description: 'Please assign all items', variant: 'destructive' });
      return;
    }
    ...
    return (
      <MainLayout>
        <div className="container mx-auto px-4 py-6 max-w-2xl">
          ...
          <h1 className="text-3xl font-bold mb-6">Custom Split</h1>
                    ...
                    {!paidPeople.includes(person) ? (
                      <div className="space-y-4">
                        <div className="flex justify-center p-4 bg-background border rounded-lg">
                          <QRCodeSVG
                            value={`upi://pay?pa=smartserve@upi&pn=SmartServe&am=${amount.toFixed(2)}&cu=INR&tn=Order Payment Person ${person}`}
                            size={200}
                            level="H"
                          />
                        </div>
  ...
  return (
    <MainLayout>
      <div className="container mx-auto px-4 py-6 max-w-2xl">
        ...
        <h1 className="text-3xl font-bold mb-6">Assign Items</h1>
          ...
          {items.map(item => (
            <Card key={item.id}>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">Total quantity: {item.quantity}</p>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-3">
                  {Array.from({ length: parseInt(numPeople) }, (_, i) => i + 1).map((person) => (
                    <div key={person}>
                      <Label htmlFor={`${item.id}-${person}`} className="text-sm">Person {person}</Label>
                      <Input
                        id={`${item.id}-${person}`}
                        type="number"
                        min="0"
                        max={item.quantity}
                        value={itemAssignments[item.id]?.[person] || 0}
                        onChange={(e) => handleItemAssignment(item.id, person, parseInt(e.target.value) || 0)}
                        className="mt-1"
                      />
                    </div>
                  ))}
            ...
            <div className="space-y-2">
              {Array.from({ length: parseInt(numPeople) }, (_, i) => i + 1).map((person) => (
                <div key={person} className="flex justify-between">
                  <span>Person {person}</span>
                  <span className="font-semibold">₹{calculatePersonTotal(person).toFixed(2)}</span>
                </div>
              ))}
            </div>
          </CardContent>
